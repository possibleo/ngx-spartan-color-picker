...
import { expandSpartanClasses, spartanStyle } from './spartan-style';

export function hlm(...inputs: ClassValue[]) {
  // Read active style so callers in reactive contexts re-run on style changes.
  spartanStyle();
  return twMerge(expandSpartanClasses(clsx(inputs)));
}
...
export function classes(computed: () => ClassValue[] | string, options: ClassesOptions = {}) {
  runInInjectionContext(options.injector ?? inject(Injector), () => {
    ...
    function updateClasses(): void {
      // Track style so token → utility expansion updates when the demo style changes.
      spartanStyle();

      // Get the new classes from the computed function
      const newClasses = toClassList(expandSpartanClasses(clsx(computed())));
